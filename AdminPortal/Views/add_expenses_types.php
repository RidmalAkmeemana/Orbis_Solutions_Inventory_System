<?php 
    ...
    // Check if user has access to addNewExpenseType.php
    $has_access_to_add_expense_type = false;
    $permission_query = mysqli_query($conn, "SELECT * FROM `tbl_backend_permissions` WHERE `Role` = '$user_status' AND `Backend_Id` = 154") or die(mysqli_error());
    if (mysqli_num_rows($permission_query) > 0) {
        $has_access_to_add_expense_type = true;
    }
?>
...
<head>
    ...
    <body>
        ...
        <div class="main-wrapper">
					...
					<div class="page-header">
						<div class="row">
							<div class="col-sm-7 col-auto">
								<h3 class="page-title">Expenses Types</h3>
								<ul class="breadcrumb">
							...
							<div class="col-sm-5 col">
								<?php if ($has_access_to_add_expense_type): ?>
                                	<a href="#Add_ExpenseType" data-toggle="modal" class="btn btn-primary float-right mt-2"> <i class="fa fa-plus-square" aria-hidden="true"></i> Add New Expense Type</a>
                            	<?php else: ?>
                                	<a style="display:none;" href="#" data-toggle="modal" class="btn btn-primary float-right mt-2"> <i class="fa fa-plus-square" aria-hidden="true"></i> Add New Expense Type</a>
                            	<?php endif; ?>
							</div>
						</div>
					</div>
			...
			<!-- Add Modal -->
			<div class="modal fade" id="Add_ExpenseType" aria-hidden="true" role="dialog">
				<div class="modal-dialog modal-dialog-centered" role="document" >
					<div class="modal-content">
						...
						<div class="modal-body">
							<form method="POST" action="../../API/Admin/addNewExpenseType.php" id="addExpenseTypeForm" enctype="multipart/form-data">
								...
								?>
									...
									<div class="col-12">
										<div class="form-group">
											...
											<select style="width:100%;" name="Expense_Category_Id" id="expenseCategorySelect" class="form-control" required="">
												<option selected disabled>Select Expenses Category</option>
													<?php
                        								if (! empty($countryResult)) 
														{
															...
															{
                                								echo '<option value="' . $countryResult[$key]['Expense_Category_Id'] . '">' . $countryResult[$key]['Expense_Category_Name'] . '</option>';
                            								}
                        								}
                        							?>
											</select>
        ...
        $.ajax({
            type: 'POST',
            url: '../../API/Admin/getAllExpenseTypeData.php',
            dataType: 'json',
                ...
                if (data.length > 0) {
                    ...
                    $.each(data, function (index, row) {
                        table.row.add([
                            ...
                            row.expense_count,
                            '<div class="actions"><a class="btn btn-sm bg-success-light" href="view_expenses_types.php?Expense_Type_Id=' + row.Expense_Type_Id + '"><i class="fe fe-eye"></i> View </a></div>'
                        ]);
                    });
		...
		// Function to add a new expensetype
		$('#addExpenseTypeForm').submit(function (event) {
			...
			$.ajax({
				type: 'POST',
				url: '../../API/Admin/addNewExpenseType.php',
				data: $(this).serialize(),
