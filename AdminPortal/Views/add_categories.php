<?php 
    ...
    // Check if user has access to addNewCategory.php
    $has_access_to_add_category = false;
    $permission_query = mysqli_query($conn, "SELECT * FROM `tbl_backend_permissions` WHERE `Role` = '$user_status' AND `Backend_Id` = 100") or die(mysqli_error());
    if (mysqli_num_rows($permission_query) > 0) {
        $has_access_to_add_category = true;
    }
?>
...
<head>
    ...
    <body>
					...
					<div class="page-header">
						<div class="row">
							...
							<div class="col-sm-5 col">
								<?php if ($has_access_to_add_category): ?>
                                	<a href="#Add_Category" data-toggle="modal" class="btn btn-primary float-right mt-2"> <i class="fa fa-plus-square" aria-hidden="true"></i> Add New Category</a>
                            	<?php else: ?>
                                	<a style="display:none;" href="#" data-toggle="modal" class="btn btn-primary float-right mt-2"> <i class="fa fa-plus-square" aria-hidden="true"></i> Add New Category</a>
                            	<?php endif; ?>
							</div>
						</div>
					</div>
				...
				<div class="modal-dialog modal-dialog-centered" role="document" >
					<div class="modal-content">
						...
						<div class="modal-body">
							<form method="POST" action="../../API/Admin/addNewCategory.php" id="addCategoryForm" enctype="multipart/form-data">
								<div class="row form-row">

									<div class="col-12">
										<div class="form-group">
											<label>Category ID</label><label class="text-danger">*</label>
        ...
        // Make an AJAX request to getAllCategoryData.php
        $.ajax({
            type: 'POST',
            url: '../../API/Admin/getAllCategoryData.php',
            dataType: 'json',
                ...
                if (data.length > 0) {
                    ...
                    $.each(data, function (index, row) {
                        table.row.add([
                            ...
                            row.Category_Name,
                            row.product_count,
                            '<div class="actions"><a class="btn btn-sm bg-success-light" href="view_category.php?Category_Id=' + row.Category_Id + '"><i class="fe fe-eye"></i> View </a></div>'
                        ]);
                    });
