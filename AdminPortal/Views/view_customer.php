					...
					<div class="row">
						<div class="col-md-12 text-left">
							<?php if ($has_access_to_edit_customer): ?>
								<a href="#Update_Customer" data-toggle="modal" class="btn btn bg-primary-light"><i class="fe fe-pencil"></i> Edit</a>
							<?php else: ?>
							...
							<?php if ($has_access_to_delete_customer): ?>
								<a href="#Delete_Customer" data-toggle="modal" class="btn btn bg-danger-light"><i class="fe fe-trash"></i> Delete</a>
							<?php else: ?>
							...
							<div class="row form-row">
								...
								<div class="col-12 col-sm-6">
									<div class="form-group">
										...
										<input style="display:none;" type="text" name="Customer_Id" class="form-control" required="" readonly="true" value="<?php echo $fetch1['Customer_Id']; ?>">
										<input type="text" name="Customer_Name" class="form-control" required="" value="<?php echo $fetch1['Customer_Name']; ?>">
									</div>
								</div>
								...
								<div class="col-12 col-sm-6">
									<div class="form-group">
										...
										<input type="text" name="Customer_Contact" class="form-control" required="" value="<?php echo $fetch1['Customer_Contact']; ?>">
									</div>
								</div>
								...
								<div class="col-12">
									<div class="form-group">
										...
										<?php	} else { ?>
											<input type="email" name="Customer_Email" class="form-control" value="<?php echo $fetch1['Customer_Email']; ?>">
										<?php	}
										?>
									</div>
								</div>
								...
								<div class="col-12">
									<div class="form-group">
										...
										<textarea id="my-text" name="Customer_Address" class="form-control" rows="8"><?php echo $fetch1['Customer_Address']; ?></textarea>
				...
				<div class="modal-content">
					...
					<div class="modal-body">
						<div class="form-content p-2">
							<h4 class="modal-title">Delete <?php echo $fetch1['Customer_Name']; ?></h4>
							<p class="mb-4">Are you sure want to delete ?</p>

							<form method="POST" action="../../API/Admin/deleteCustomer.php" id="deleteCustomerForm" enctype="multipart/form-data">
								<input style="display: none;" type="text" name="Customer_Id" value="<?php echo $fetch1['Customer_Id']; ?>">
				...
				function fetchCustomerData(Customer_Id) {
					$.ajax({
						...
						success: function(response) {
							...
							const cutomerAddress = response.customerData.Customer_Address ? response.customerData.Customer_Address : 'N/A';
							const cutomerEmail = response.customerData.Customer_Email ? response.customerData.Customer_Email : 'N/A';

							$('#customerName').text(response.customerData.Customer_Name);
							$('#customerId').text(response.customerData.Customer_Id);
							$('#customerAddress').text(cutomerAddress);
							$('#customerContact').text(response.customerData.Customer_Contact);
							$('#customerEmail').text(cutomerEmail);
							...
							if (response.invoices.length > 0) {
								$.each(response.invoices, function(index, invoice) {
									...
									var hasAccessToPrintInvoice = <?php echo json_encode($has_access_to_print_invoice); ?>;

									const formattedGrandTotal = 'LKR: ' + invoice.Grand_Total;
									...
									let actions = '';
									if (hasAccessToPrintInvoice) {
										actions = '<div class="actions"><a class="btn btn-sm bg-success-light" href="view_invoice.php?Invoice_No=' + invoice.Invoice_Id + '"><i class="fe fe-eye"></i> View </a></div>';
									} else {
